...
| C — Operational breadth | Intraday reallocation, training/certification, leave/RDO; WMS live backlog integration | **In progress** — policy governance done first (`app/core/policy.py`); `training_coverage` (MILP) is the first new model, done and tested. Leave/RDO, intraday reallocation, and WMS integration not started | `services/tempo-api/app/solvers/training_coverage.py` |
