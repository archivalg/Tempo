...
| C — Operational breadth | Intraday reallocation, training/certification, leave/RDO; WMS live backlog integration | **In progress** — policy governance, `training_coverage`, `leave_rdo` (MILP), and `intraday_reallocation` (min-cost flow, a new solver family) all done and tested. Only WMS live-backlog integration remains — `intraday_reallocation`'s new `ZoneBacklog` table currently only accepts seeded/direct-insert data | `services/tempo-api/app/solvers/` |
