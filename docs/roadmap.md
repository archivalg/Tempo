...
| C — Operational breadth | Intraday reallocation, training/certification, leave/RDO; WMS live backlog integration | **In progress** — policy governance done first (`app/core/policy.py`); `training_coverage` and `leave_rdo` (both MILP) done and tested. Intraday reallocation (min-cost flow) and WMS integration not started | `services/tempo-api/app/solvers/` |
